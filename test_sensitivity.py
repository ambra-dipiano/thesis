...
if plot:
  ...
  for i in range(len(caldb)):
    for j in range(len(texp)):
      ...
      savefig2.append(pngpath + pngroot + 'sensdiff_phflux.png')
      savefig3.append(pngpath + pngroot + 'sensdiff_ratio.png')
  ...
  for j in range(len(texp)):
    ...
    showSensitivity([10**energy_nom, 10**energy_deg], [sens_nom, sens_deg], savefig1[j], marker=['+', 'x'],
                    xlabel='energy (TeV)', ylabel='E$^2$F sensitivity (erg/cm$^2$/s)',
                    label=['full sens irf', 'degraded irf'], title=title, fontsize=12, show=False)

    showSensitivity([10**energy_nom, 10**energy_deg], [flux_nom, flux_deg], savefig2[j], marker=['+', 'x'],
