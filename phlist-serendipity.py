...
caldb = 'prod3b-v2'
irf = 'South_z40_0.5h'
tburst = 80  # burst time wrt obs start time (s)
...
bkg_model = p.getModelsDir() + 'CTAIrfBackground.xml'
...
print('!!! *** !!! burst time:', tburst, ' s')
print('!!! *** !!! total observation time:', ttotal + tburst, ' s')
...
tgrid = simulation.getTimeSlices(GTI=(0, ttotal))  # methods which returns time slice edges ---!
# simulate ---!
...
# ----------------------------- SHIFT GRB TEMPLATE IN TIME --------------------- !!!
print('Shift template of', tburst, 's after the start of the observation')
simulation.shiftTemplateTime(phlist=event_bins, time_shift=tburst)

# --------------------------------- SIMULATION BKG --------------------------------- !!!
print('Simulate bkg to add before the burst')
bkg = p.getObsDir() + 'bkg_' + f + '.fits'
event_bins.insert(0, bkg)
simulation.t = [0, tburst]
simulation.model = bkg_model
simulation.output = bkg
simulation.eventSim()
...
if not use_run:
    ...
    simulation.appendEventsSinglePhList(GTI=[0, ttotal + tburst])
    phlist = event_list
    ...
    simulation.input = event_bins
    simulation.output = event_list
    num_max, phlist = simulation.appendEventsMultiPhList(max_length=run_duration, last=ttotal + tburst)
