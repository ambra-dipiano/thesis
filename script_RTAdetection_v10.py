...
for k in range(trials) :
  ...
  # ====================================================
  # !!! LIKELIHOOD RA & DEC AND CORRESPONDING ERRORS !!!
  # ====================================================
  ...
  for i in range(tint) :
    if Ndet[i] > 0 :
      ...
      flux_erg.append(cost_erg * integ)  # convert E (erg)
  ...
  csvName = []
  header = '#trial,t exp,sigma,Ndet,Nsrc,RA Src001,DEC Src001,RA Fit,DEC Fit,RA err,DEC err,TSV,flux1 ph,flux2 ph,flux1 erg,flux2 erg\n'
  ID = 'ID%06d' % count
  ...
  for i in range(tint):
    ...
    row = []
    row.append([ID,texp[i],sigma,Ndet[i],Nsrc[i],raSrc001[i],decSrc001[i],raFit[i],decFit[i],raFit_err[i],decFit_err[i],tsv[i],flux[i],flux_ph[i],flux_erg[i],flux_en[i]])
...
print('!!! check end\n\ndone......chunk ', chunk, 'sim id from ', trials*(chunk-1)+1+4 , ' to ', count)
