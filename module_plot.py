...
def showSkymap(file, reg='none', col='black', suffix='none', title='skymap',
               xlabel='R.A. (deg)', ylabel='Dec (deg)', fontsize=12, show=True, tex=True):
...
def showResiduals(file, yscale='log', title='spectral residuals', figsize=(10,8),
                  xlabel='energy (TeV)', ylabel=('counts', 'residuals'), fontsize=12, show=True, tex=True):
  with fits.open(file) as hdul:
    data = hdul[1].data
    # store data ---!
    Emin = data.field(0)
...
def showButterfly(file, flux_pnts=0.0, fluxEn_pnts=0.0, suffix='none', title='flux', fontsize=12,
                  xlabel='Energy (TeV)', ylabel='E $\cdot \\frac{dN}{dE}$ (erg/$cm^2$/s)', show=True, tex=True):
...
def showSpectrum(file, figsize=(8,15), fontsize=12, title=('spectrum with errors', 'spectrum with errors', 'log spectrum'),
                 xlabel='Energy (TeV)', ylabel='Flux (erg/$cm^2$/s)', show=True, tex=True):
  with fits.open(file) as hdul:
    data = hdul[1].data
    # store data ---!
    en = data.field(0)
...
def showTSmap(file, reg='none', col='black', suffix='none', title='TS map', cbar='Significance TSV',
              xlabel='RA (deg)', ylabel='DEC (deg)', fontsize=12, show=True, tex=True):
  with fits.open(file) as hdul:
    data = hdul[0].data
    hdr = hdul[0].header
...
def degradedIRF_3d(x, y, z, xlabel='x', ylabel='y', zlabel='z', title=None, c=('b'), zscale='linear', tex=True,
...
def interp_ebl(x, y, savefig, type='linear', xlabel='x', ylabel='y', title='title',
               label=('y', 'y2'), fontsize=12, show=True, tex=True):
