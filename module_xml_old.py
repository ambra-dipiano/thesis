...
def srcDetection_spcModeling(skymap, sigma=5, instr='CTA', bkgType='Irf', src_attrib='none', bkg_attrib='none', tsv=True, maxSrc=20, exclrad = 0.5) :
  ...
  detectionXml_mod = detectionXml.replace('.xml', '_Mod.xml')
  # write new file ---!
  srcLib.write(detectionXml_mod, encoding="UTF-8", xml_declaration=True,
               standalone=False, pretty_print=True)

  file.close()

  return detectionXml_mod, detectionReg, posList
